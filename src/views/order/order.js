...
if (pg_token) {
    ...
    Api.post(`${location.protocol}//${location.host}/api/order/register`, Storage.get('order'));
    Storage.clear('order');
...
function renderProductsList(products) {
    try {
        products.forEach(({ amount, product, _id }) => {
            ...
            $price.innerHTML = `${(product.price * amount).toLocaleString()}<span>원</span>`;
...
function getTotalPrice(data) {
    let total;
    try {
        total = data.reduce((price, { amount, product }) => price + amount * product.price, 0);
    } catch (e) {
        alert(e.message);
    }
...
async function getData() {
    let data = await Api.get(`${location.protocol}//${location.host}`, 'api/cart');
    ...
    const paymentType = {
        card: () => {
            ...
            IMP.request_pay(
                ...
                (res) => {
                    if (res.success) {
                        Api.post(`${location.protocol}//${location.host}/api/order/register`, formData);
                        window.location.replace('/order/result');
                    } else {
                        alert('결제 실패');
                    }
        ...
        kakao: () => {
            ...
            urlencoded.append('approval_url', `${location.protocol}//${location.host}/order`);
            urlencoded.append('fail_url', `${location.protocol}//${location.host}/order?result=fail`);
            urlencoded.append('cancel_url', `${location.protocol}//${location.host}/order`);
